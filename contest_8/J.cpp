...
class Graph {
 public:
  ...
  void Construct(std::string str1, std::string str2, int start, int end) {
    for (size_t i = 0; i < str2.size(); ++i) {
      for (size_t j = i; j + str2.size() - i <= str1.size(); ++j) {
        if (str1[j] == '?' || str2[i] == '?') {
          if (str1[j] == '0' && str2[i] == '?') {
            AddEdge(start, 2 * i, 1);
            ...
            AddEdge(2 * i, 1 + 2 * j, 1, 1);
          } else if (str1[j] == '?' && str2[i] == '0') {
            AddEdge(start, 2 * j + 1, 1);
          } else if (str1[j] == '?' && str2[i] == '1') {
            AddEdge(2 * j + 1, end, 1);
          }
...
int main() {
  ...
  int count_vertex = 2 * (first.size() + second.size()) + 2;
  int start = count_vertex - 2;
  int end = count_vertex - 1;
  Graph graph(count_vertex);
  graph.Construct(first, second, start, end);
  int64_t res = graph.Dinic(start, end);
  std::cout << res << '\n';
  graph.Result(start);
  graph.PrintRes(first, second);
  return 0;
}